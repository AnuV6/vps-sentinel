...
export async function checkSite(monitor: any) {
    ...
    // Log to DB
    await db.run(
        `INSERT INTO checks (monitor_id, status, latency) VALUES (?, ?, ?)`,
    ...
    await db.run(
        `UPDATE monitors SET last_check_status = ?, last_check_latency = ?, last_check_time = datetime('now','localtime') WHERE id = ?`,
